...
pub async fn main() {
    ...
    let unit = args.get(3).expect("Missing unit").parse().unwrap();
    let min_value = args
        .get(4)
    ...
    let handle = tokio::task::spawn(async move {
        create_iot_sensor(sensor_type, sensor_name, unit, min_value, max_value).await
    });

    tokio::join!(handle).0.unwrap();
}

async fn create_iot_sensor(
    sensor_type: String,
    sensor_name: String,
    unit: String,
    min: f32,
    ...
    loop {
        ...
        if sensor_reading < min || sensor_reading > max {
            broker
                .send_message(&format!(
                    "{sensor_type}.{sensor_name}: {sensor_reading:.2}{unit}"
                ))
                .await;
        }
